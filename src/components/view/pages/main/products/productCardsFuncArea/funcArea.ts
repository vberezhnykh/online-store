...
class FuncArea {
  sort(e: Event) {
    ...
    if (option !== "Sort_options") {
      ...
      for (let i = 0; i < categories.length; i++) {
        ...
        obj.order = Number(
          (parent.closest(".card__wrapper") as HTMLDivElement).style.order
        );
      ...
      const sortedArr =
        sortType === "ASC"
          ...
          : quickSort(arr, sortType).reverse();
      ...
      for (let i = 0; i < cards.length; i++) {
        cards[i].style.order = `${
          sortedArr.findIndex(
            (obj) => Number(cards[i].style.order) === obj.order
          ) + 1
        }`;
      }
    }
  ...
  createSort() {
    const sort = <HTMLSelectElement>(
      createCustomElement({ selector: "select", class: "card__sort" })
    );
    Object.values(types.CardSort).forEach((element, i) => {
      if (typeof element === "string") {
        const option = <HTMLOptionElement>(
          createCustomElement({ selector: "option", class: "sort__option" })
        );
  ...
  createFind() {
    const found = <HTMLDivElement>(
      createCustomElement({ selector: "div", class: "card__stat" })
    );
    const foundText = <HTMLSpanElement>(
      createCustomElement({ selector: "span", class: "stat__text" })
    );
    const foundCount = <HTMLSpanElement>(
      createCustomElement({ selector: "span", class: "stat__count" })
    );
  ...
  createSearch() {
    const search = <HTMLInputElement>(
      createCustomElement({ selector: "input", class: "card__search" })
    );
    search.type = "search";
    ...
    search.addEventListener("keyup", () => {
      ...
      let count = 0;
      for (let i = 0; i < cards.length; i++) {
        const values = cards[i].querySelectorAll(
          ".item__category-value"
        ) as NodeListOf<HTMLSpanElement>;
        let result = false;
      ...
      (
        document.querySelector(".stat__count") as HTMLSpanElement
      ).textContent = `${count}`;
    });
    return search;
  }
  ...
  createDisplay() {
    const displayArea = <HTMLDivElement>(
      createCustomElement({ selector: "div", class: "card__display-area" })
    );
    const displaySmall = <HTMLDivElement>(
      createCustomElement({ selector: "div", class: "card__display" })
    );
    const displayLarge = <HTMLDivElement>(
      createCustomElement({ selector: "div", class: "card__display" })
    );
    for (let i = 0; i < 16; i += 1) {
      const displayLargeDot = <HTMLDivElement>(
        createCustomElement({ selector: "div", class: "large__dot" })
      );
      displayLargeDot.textContent = ".";
      displayLarge.appendChild(displayLargeDot);
    }
    for (let i = 0; i < 36; i += 1) {
      const displaySmallDot = <HTMLDivElement>(
        createCustomElement({ selector: "div", class: "small__dot" })
      );
  ...
  createCardFuncsArea() {
    const funcArea = <HTMLDivElement>(
      createCustomElement({ selector: "div", class: "product__funcs" })
    );
  ...
  draw() {
    ...
    ).appendChild(this.createCardFuncsArea());
  }
}
