...
class CartPage {
  product: CartProduct;
  summary: CartSummary;
  _productsInCart: Array<IProductInfo> = [];

  constructor(productsInCart: Array<IProductInfo>) {
  ...
  fillTotals() {
    const totalAmount = Array.from(
      document.querySelectorAll(".item__count")
    ).reduce((sum, curr) => sum + Number(curr.textContent), 0);
    ...
    (
      document.querySelector(".total-amount") as HTMLSpanElement
    ).textContent = `${totalAmount}`;
    (document.querySelector(".header__cart") as HTMLDivElement).setAttribute(
      "data-value",
      `${totalAmount}`
    );
    (
      document.querySelector(".total-price") as HTMLSpanElement
    ).textContent = `$${totalPrice}`;
    (
      document.querySelector(".total__sum") as HTMLSpanElement
    ).textContent = `${totalPrice}`;
  }

  hideCartItems(page = 1) {
    const limit = Number(
      (document.querySelector(".limit__amount") as HTMLInputElement).value
    );
    Array.from(document.querySelectorAll(".cart__items")).forEach((el, i) => {
      (el as HTMLDivElement).style.display =
        i >= limit * (page - 1) && i < limit * page ? "" : "none";
    });
  }
      ...
      area.addEventListener("click", (event) => {
        ...
        if (target.className.includes("page__button")) {
          ...
          const maxPage = Math.ceil(
            document.querySelectorAll(".list__item").length / limit
          );
          let currPage = Number(
            document.querySelector(".page__amount")?.textContent
          );
          ...
          (
            document.querySelector(".page__amount") as HTMLSpanElement
          ).textContent = `${currPage}`;
        }
        if (target.className.includes("item__button")) {
          ...
          if (parent) {
            const stock = Number(
              (
                parent.querySelector(".item__stock") as HTMLSpanElement
              ).textContent
                ?.split(" ")
                .reverse()[0]
            );
            const itemTitle = (
              parent.querySelector(".title__text") as HTMLHeadingElement
            ).textContent;
            const price = PRODUCTS.filter((item) =>
              item.title.toLowerCase().includes(`${itemTitle}`.toLowerCase())
            )[0].price;
            let amount = Number(
              parent.querySelector(".item__count")?.textContent
            );
            const step = target.className.includes("plus") ? 1 : -1;
            if (amount + step > 0 && amount + step <= stock) {
              amount += step;
              (
                parent.querySelector(".item__count") as HTMLSpanElement
              ).textContent = `${amount}`;
              (
                parent.querySelector(".item__price") as HTMLSpanElement
  ...
  draw() {
    const container = <HTMLDivElement>(
      createCustomElement({ selector: "div", class: "cart__page" })
    );
    (document.querySelector(".page__container") as HTMLDivElement).appendChild(
      container
    );
    this.product.draw(this._productsInCart);
    this.summary.draw(this._productsInCart);
    this.fillTotals();
